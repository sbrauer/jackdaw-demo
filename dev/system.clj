...
(defn stop
  ...
  (re-delete-topics (re-pattern (str "("
                                     (->> demo/topic-metadata
                                          ...
                                          (str/join "|"))
                                     "|"
                                     (application-id demo/app-config)
                                     ".*)")))
  (destroy-state-stores demo/app-config))
...
(defn start
  ...
  []
  (with-out-str (stop))
  (create-topics (vals demo/topic-metadata))
  {:app (demo/start-app demo/topic-metadata demo/app-config)})
